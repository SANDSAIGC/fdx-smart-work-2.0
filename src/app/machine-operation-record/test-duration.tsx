...
export default function TestDuration() {
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6">持续时长计算测试</h1>
      
      <div className="space-y-4">
        <h2 className="text-lg font-semibold">原始测试数据：</h2>
        <div className="grid gap-2">
          {testRecords.map((record, index) => (
            <div key={record.id} className="p-3 border rounded">
              <span className="font-mono text-sm">
                {index + 1}. {record.日期} {record.时间} - {record.设备状态} - {record.操作员}
              </span>
            </div>
          ))}
        </div>

        <h2 className="text-lg font-semibold mt-8">计算结果（按时间倒序）：</h2>
        <div className="grid gap-2">
          {recordsWithDuration.map((record, index) => (
            <div key={record.id} className="p-3 border rounded">
              <div className="font-mono text-sm">
                {record.日期} {record.时间} - {record.设备状态} - {record.操作员}
              </div>
              <div className="text-sm text-muted-foreground mt-1">
                持续时长: <span className="font-semibold">{record.持续时长 ?? '--'}</span>
              </div>
            </div>
          ))}
        </div>

        <h2 className="text-lg font-semibold mt-8">预期结果验证：</h2>
        <div className="text-sm space-y-1">
          <p>• 2025-07-03 08:00 (最新记录): 持续时长应为 "--"</p>
          <p>• 2025-07-02 18:30: 持续时长应为 "13小时30分钟" (到次日08:00)</p>
          <p>• 2025-07-02 16:00: 持续时长应为 "2小时30分钟" (到18:30)</p>
          <p>• 2025-07-02 12:00: 持续时长应为 "4小时" (到16:00)</p>
          <p>• 2025-07-02 08:00: 持续时长应为 "4小时" (到12:00)</p>
        </div>
      </div>
    </div>
  );
