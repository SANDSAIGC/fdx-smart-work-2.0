...
export default function MachineOperationRecordPage() {
  ...
  const updateDurations = async () => {
    try {
      const response = await fetch('/api/machine-operation-record/update-durations', {
        method: 'POST'
      });

      if (!response.ok) {
        console.warn('更新持续时长失败');
      }
    } catch (error) {
      console.warn('更新持续时长失败:', error);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const recordData = {
        ...
        持续时长: null, // 持续时长在前端动态计算，新记录初始为null
        操作员: user.name,
