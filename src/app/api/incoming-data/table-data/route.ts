...
export async function POST(request: NextRequest) {
  try {
    ...
    const queryUrl = `${supabaseUrl}/rest/v1/${encodeURIComponent(tableName)}?select=*&${dateField}=gte.${startDate}&${dateField}=lte.${endDate}&order=${dateField}.desc`;
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        console.log(`获取表格数据第${retryCount + 1}次尝试失败:`, error);
        retryCount++;
    ...
    if (!response || !response.ok) {
      console.error('获取表格数据失败:', response?.status, response?.statusText);
      return NextResponse.json(
